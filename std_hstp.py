...
def run_simulation(i):
    ...
    bar = tqdm(total=sims*2, position=i, desc=f"Simulation {i:02}", leave=True, ncols=80)
    ...
    hold_output_file = "timing/hold_"+str(lic)+"_"+str(ic)+".csv"
    setup_output_file = "timing/setup_"+str(lic)+"_"+str(ic)+".csv"
...
if __name__ == "__main__":
    for i in range(1, 19 + 1):
        open(f"timing/hold_"+str(lic)+"_"+str(5*(i+1))+".csv", "w").close()
        open(f"timing/setup_"+str(lic)+"_"+str(5*(i+1))+".csv", "w").close()
